...
static node_t *ht_get_node(hashtable_t*,const void*);
static node_t *ht_get_node_i(hashtable_t*,const void*,uint32_t);
...
void *ht_add(hashtable_t *t, const void* key, void *val){
...
void *ht_get(hashtable_t *t, const void* key){
	node_t *n = ht_get_node(t, key);
	return n?n->kvp.val:NULL;
}

node_t *ht_get_node(hashtable_t *t, const void* key){
	return ht_get_node_i(t, key, t->hash_code(key) & (t->cap - 1));
}

node_t *ht_get_node_i(hashtable_t *t, const void* key, uint32_t index){
	node_t *n;
...
void subrehash(hashtable_t *t, node_t **newarr, size_t newcap, node_t *n){
	if(n->next){
